...
def get_report_categories(conn=None):
    ...
    """
    return request_json(
        endpoint_url("reporting", "report-categories", conn=conn), conn=conn
    )
...
def get_report_list(report_category, conn=None):
    ...
    return request_json(
        endpoint_url(
            "reporting",
            f"report-category/{report_category}/reports",
            conn=conn,
        ),
        conn=conn,
    )
...
def get_dynamic_set_list(dynamic_set_id, conn=None):
    ...
    return request_json(
        endpoint_url(
            "reporting", f"dynamic-value-sets/{dynamic_set_id}", conn=conn
        ),
        conn=conn,
    )
...
class Report:
    ...
    def add_params(self, name, value):
        ...
        if name in param_keys:
            logger.info(
                f"Parameter '{name}' already exists. Updating value from '{self.params['parameters'][param_keys.index(name)]['value']}' to '{value}'..."
            )
    ...
    def update_params(self, name, value):
        ...
        if name in param_keys:
            ...
            logger.info(
                f"Parameter '{name}' does not exist. Adding parameter..."
            )
    ...
    def show_param_types(self):
        ...
        for param in self.metadata["parameters"]:
            ...
            if param["acceptValues"]:
                dynamic_set_id = (
                    f" (dynamicSetId: {param['acceptValues']['dynamicSetId']})"
                )
                if param["acceptValues"]["values"]:
                    accepted_values = param["acceptValues"]["values"]
            logger.info(
                f"{required} - {param['name']}: {param['dataType']}, {dynamic_set_id}"
            )
    ...
    def get_data(self, params="", page=1, page_size=5000):
        ...
        endpoint = (
            f"report-category/{self.category}/reports/{self.report_id}/data"
        )
        url = endpoint_url("reporting", endpoint, conn=self.conn)
    ...
    def get_all_data(self, params="", page_size=5000, timeout_min=60):
        ...
        if mins_to_complete > timeout_min:
            ...
            if page_size < 5000 and math.ceil(total / 5000) < 12:
                logger.info(
                    "Setting page size to 5000 to speed up report retrieval..."
                )
                updated_page_size = 5000
                requests_needed = 1 + math.ceil(
                    (total - init_page_size) / updated_page_size
                )
            else:
                logger.warning(
                    f"This request will take at least {mins_to_complete/60} hours to complete."
                )
                logger.warning(
                    "Limit the parameters to reduce the number of requests and try again."
                )
                return {
        ...
        while has_more:
            ...
            response = self.get_data(
                params, page=page, page_size=updated_page_size
            )
