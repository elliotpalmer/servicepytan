...
class Endpoint:
    ...
    def get_many(self, query={}, id="", modifier=""):
        ...
        options = check_default_options(query)
        return request_json(
            url, options, payload="", conn=self.conn, request_type="GET"
        )
    ...
    def get_all(self, query={}, id="", modifier=""):
        ...
        while True:
            ...
            response = self.get_many(
                query=current_query, id=id, modifier=modifier
            )
    ...
    def delete(self, id, modifier=""):
        ...
        return request_json(
            url, options={}, payload="", conn=self.conn, request_type="DEL"
        )
    ...
    def delete_subitem(self, id, modifier_id, modifier):
        ...
        url = endpoint_url(
            ...
            self.endpoint,
            id=id,
            modifier=f"{modifier}/{modifier_id}",
            conn=self.conn,
        )
        return request_json(
            url, options={}, payload="", conn=self.conn, request_type="DEL"
    ...
    def export_one(
        self, export_endpoint, export_from="", include_recent_changes=False
    ):
        ...
        return request_json(
            ...
            options={
                "from": export_from,
                "includeRecentChanges": include_recent_changes,
            },
            payload="",
            conn=self.conn,
            request_type="GET",
    ...
    def export_all(
        ...
        response = self.export_one(
            export_endpoint, export_from, include_recent_changes
        )
        data = response["data"]
        if data == []:
            return []
