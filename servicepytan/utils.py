...
def request_json(
    url, options={}, payload={}, conn=None, request_type="GET", json_payload={}
):
    ...
    if response.status_code != requests.codes.ok:
        logger.error(
            f"Error fetching data (url={url}, heads={headers}, data={payload}, json={json_payload}): {response.text}"
        )
...
def endpoint_url(
    folder, endpoint, id="", modifier="", conn=None, tenant_id=""
):
...
def request_json_with_retry(
    ...
    if "traceId" in response:
        if response["status"] == 429:
            ...
            logger.warning(
                "Rate Limit Exceeded. Retrying in {} seconds...".format(
                    sleep_time
                )
            )
...
def request_contents(url, options={}, conn=None):
    ...
    response = requests.get(
        url, params=options, headers=get_auth_headers(conn)
    )
    response.raise_for_status()

    if response.status_code != requests.codes.ok:
        logger.error(
            f"Error fetching contents (url={url}, options={options}): {response.text}"
        )
        response.raise_for_status()
